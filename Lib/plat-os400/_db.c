 ...
 * 
 *--------------------------------------------------------------------
 * Copyright (c) 2002 by Per Gummedal.
 *
 * p.g@figu.no
 * 
...
servermode (default = True), run all sql commands in a server job,\n\
...
connection_init(PyObject *self, PyObject *args, PyObject *keywds)
{
    ...
    if (rc == SQL_SUCCESS) {
        ...
        if (rc == SQL_SUCCESS && database != NULL) {
            /* default database */
            rc = SQLSetEnvAttr(c->henv, SQL_ATTR_DEFAULT_LIB, database,
                               strlen(database));
        }
    ...
    if (rc == SQL_SUCCESS)
        /* connect */
        rc = SQLConnect (c->hdbc, dsn, SQL_NTS, user, SQL_NTS, pwd, SQL_NTS);
    if (rc != SQL_SUCCESS) {
...
#pragma exception_handler(EXCP1, excpData, 0, _C2_MH_ESCAPE, _CTLA_HANDLE)
    ...
    if (size == SQL_NULL_DATA) {
        ...
        return PyUnicode_FromWideChar((wchar_t *)p, size);
    }
 EXCP1:
...
init_db(void)
{
    ...
    m = Py_InitModule3("_db", db_memberlist, db_doc);
    d = PyModule_GetDict(m);
    dbError = PyErr_NewException("_db.Error", NULL, NULL);
    dbWarning = PyErr_NewException("_db.Warning", NULL, NULL);
    ...
    if (PyErr_Occurred() ) {
        Py_FatalError("Can not initialize _db");
    }
    return;
}
