...
#TODO: Note, lib dir currently used is /src/py5.git/lib. (cp issues)
...
try: 
    ...
    print "Not running on as500."
    import os
    ##os.system("""system "sndmsg 'hi' tousr(ken)" """)

    class OS400Call(object):
        def __call__(self, cmd, len=0): 
...
execCmd('crtlib %s' % (dftLibrary))
print('Created lib')

def crtCModule(module, path, lib=None, incDir='', opt=30, 
        inline="*ON *AUTO", dbg="*NONE", tgtRls="*CURRENT"):
...
def crtSrvPgm(srvPgm, modules=None, bndSrvPgm=None, lib=None, 
        tgtRls="*CURRENT"):
    if not lib: lib=targetLibrary
...
moduleMods ={'ITERTOOLSM':'ITERTOOLSMODULE.C','CSV':'_CSV.C','RANDOM':'_RANDOMMODULE.C',
    ...
    'ERRNOMODUL':'ERRNOMODULE.C','FCNTLMODUL':'FCNTLMODULE.C',
    #FIXME: Add back? 'MD5C':'MD5C.C',
  ...
  # REQUIRED
    'PYTHON':'PYTHON.C',
    #FIXME: Add back? 'CONFIG':'CONFIG.C',
    'MAIN':'MAIN.C','ARRAYMODUL':'ARRAYMODULE.C',
    'CMATHMODUL':'CMATHMODULE.C','GETPATH':'GETPATH.C','MATHMODULE':'MATHMODULE.C',
    'STROPMODUL':'STROPMODULE.C',
    #FIXME: Add back? 'STRUCTMODU':'STRUCTMODULE.C',
...
pythonMods = {'BLTINMODUL':'BLTINMODULE.C','CEVAL':'CEVAL.C','CODECS':'CODECS.C',
    'COMPILE':'COMPILE.C','ERRORS':'ERRORS.C',
    #FIXME:  Added back? 
    # 'EXCEPTIONS':'EXCEPTIONS.C',
    'FROZEN':'FROZEN.C',
    'FROZENMAIN':'FROZENMAIN.C','GETARGS':'GETARGS.C','GETCOMPILE':'GETCOMPILER.C',
    'GETCOPYRIG':'GETCOPYRIGHT.C',
    #TODO: Add back? 'GETMTIME':'GETMTIME.C',
    'GETPLATFOR':'GETPLATFORM.C',
...
as400Mods = {'FILE400MOD':'FILE400MODULE.C',
    ...
    'DYNLOAD_AS':'DYNLOAD_AS400.C',
    #FIXME: Different path. 'PYTHONCMD':'PYTHONCMD.C',
    'ZDB':'_DB.C',
...
if len(argv) == 3:
    ...
    targetLibrary, tgtRls = dftLibrary, '*CURRENT'
optimize = 10

#TODO: Move the source to a reasonable path!
sourcePath = '~/py5.git'
sourcePath = '/src/py5.git'

#TODO: Colon seperated or no?  Seems like PASE yes, ILE no, that right?
#include =("""'%s' '%s/AS500' '%s/lib/plat-os400' '%s/Include' '%s/Modules' '%s/Objects' \
