...
static PyObject* getSQLCHARAsPyUnicodeObject(SQLCHAR* SQLCHARData, int SQLCHARBytesLen);
...
//	SQLUINTEGER file_options;	 /* File options if PARAM_FILE */
	...
	SQLCHAR *uvalue;			 /* Temp storage value */
...
static int _python_ibm_db_assign_options( void *handle, int type, long opt_key, PyObject *data )
{
	...
	if (opt_key == ATTR_CASE) {
		...
		if (type == SQL_HANDLE_STMT) {
			...
			if (opt_key!=SQL_ATTR_ROWCOUNT_PREFETCH) {	
				option_num = NUM2LONG(data);
				if (opt_key==SQL_ATTR_CURSOR_TYPE) {
				   if (option_num==SQL_CURSOR_KEYSET_DRIVEN) option_num=SQL_CURSOR_STATIC;
				}
				...
				vparm=option_num;
				rc = SQLSetStmtAttr((SQLHSTMT)((stmt_handle *)handle)->hstmt, opt_key, &vparm, SQL_IS_INTEGER );
				if ( rc == SQL_ERROR ) {
					_python_ibm_db_check_sql_errors((SQLHSTMT)((stmt_handle *)handle)->hstmt, SQL_HANDLE_STMT, rc, 1, NULL, -1, 1);
	 		        }
			}
			...
			if (opt_key!=SQL_ATTR_ROWCOUNT_PREFETCH) {
				...
				vparm=option_num;
				rc = SQLSetConnectAttr((SQLHSTMT)((conn_handle*)handle)->hdbc, opt_key, &vparm, SQL_IS_INTEGER);
				if ( rc == SQL_ERROR ) {
					_python_ibm_db_check_sql_errors((SQLHSTMT)((stmt_handle *)handle)->hstmt, SQL_HANDLE_STMT, rc, 1, NULL, -1, 1);
				}
			}
...
//				row_data->w_val = (SQLCHAR *) ALLOC_N(SQLCHAR, in_length);
				row_data->str_val = (SQLCHAR *) ALLOC_N(SQLCHAR, in_length);
//				rc = SQLBindCol((SQLHSTMT)stmt_res->hstmt, (SQLUSMALLINT)(i+1),
//					SQL_C_WCHAR, row_data->w_val, in_length * sizeof(SQLCHAR),
//					(SQLINTEGER *)(&stmt_res->row_data[i].out_length));
				rc = SQLBindCol((SQLHSTMT)stmt_res->hstmt, (SQLUSMALLINT)(i+1),
					SQL_C_CHAR, row_data->str_val, in_length * sizeof(SQLCHAR),
					(SQLINTEGER *)(&stmt_res->row_data[i].out_length));
...
//			case SQL_LONGVARBINARY:
			case SQL_VARBINARY:
				if ( stmt_res->s_bin_mode == CONVERT ) {
					...
					Py_BEGIN_ALLOW_THREADS;
//					rc = SQLBindCol((SQLHSTMT)stmt_res->hstmt, (SQLUSMALLINT)(i+1),
					rc = SQLBindCol((SQLHSTMT)stmt_res->hstmt, (SQLSMALLINT)(i+1),
...
//				(void *)SQL_OV_ODBC3, 0);
		        vparm = SQL_TRUE;
		        rc = SQLSetEnvAttr((SQLHENV)conn_res->henv, SQL_ATTR_UTF8, &vparm, 0);
			vparm = SQL_SEP_COLON;
		        rc = SQLSetEnvAttr((SQLHENV)conn_res->henv, SQL_ATTR_TIME_SEP, &vparm, 0);			
			vparm = SQL_TRUE;
		        rc = SQLSetEnvAttr((SQLHENV)conn_res->henv, SQL_ATTR_SERVER_MODE, &vparm, 0);			
		}
...
//			(SQLPOINTER)(conn_res->auto_commit), SQL_NTS);
    	        vparm = SQL_AUTOCOMMIT_ON;
	        rc = SQLSetConnectAttr((SQLHDBC)conn_res->hdbc, SQL_ATTR_AUTOCOMMIT, &vparm, 0);

    	        vparm = SQL_FALSE;
	        rc = SQLSetConnectAttr((SQLHDBC)conn_res->hdbc, SQL_ATTR_UCS2, &vparm, 0);
		...
		if (! reused) {
			...
			database = getUnicodeDataAsSQLCHAR(databaseObj, &isNewBuffer);		
...
//					SQL_DRIVER_NOPROMPT );
				rc = SQLDriverConnect((SQLHDBC)conn_res->hdbc, (SQLPOINTER)NULL,
					database, SQL_NTS, buffer, 255, &outlen, 
					SQL_DRIVER_NOPROMPT );
				...
				uid = getUnicodeDataAsSQLCHAR(uidObj, &isNewBuffer);
				password = getUnicodeDataAsSQLCHAR(passwordObj, &isNewBuffer);
...
//			if (!strcmp(server, "AS")) is_systemi = 1;
			if (!strcmp(server, "QSQ")) is_systemi = 1;
...
/**
*This function takes a SQLCHAR buffer (UCS-2) and returns back a PyUnicode object 
* of it that is in the correct current UCS encoding (either UCS2 or UCS4)of
* the current executing python VM
*
* @SQLCHARBytesLen - the length of SQLCHARData in bytes (not characters)
**/
static PyObject* getSQLCHARAsPyUnicodeObject(SQLCHAR* SQLCHARData, int SQLCHARBytesLen)
{
...
/*	
	...
	if (maxuniValue <= 65536) {
		return PyUnicode_FromUnicode((Py_UNICODE *)SQLCHARData, SQLCHARBytesLen / sizeof(SQLCHAR));
    	}
	
	if (is_bigendian()) {
		int bo = 1;
		u = PyUnicode_DecodeUTF16((char *)SQLCHARData, SQLCHARBytesLen, "strict", &bo);
	} else {
		int bo = -1;
		u = PyUnicode_DecodeUTF16((char *)SQLCHARData, SQLCHARBytesLen, "strict", &bo);
	}
*/	
	u = PyUnicode_DecodeUTF8((char *)SQLCHARData, SQLCHARBytesLen, "strict");
	return u;
}


/**
*This function takes value as pyObject and convert it to SQLCHAR and return it 
*
**/
static SQLCHAR* getUnicodeDataAsSQLCHAR(PyObject *pyobj, int *isNewBuffer)
{	
...
//#endif
				vparm=autocommit;
				rc = SQLSetConnectAttr((SQLHDBC)conn_res->hdbc, SQL_ATTR_AUTOCOMMIT, &vparm, SQL_IS_INTEGER);
				if ( rc == SQL_ERROR ) {
					_python_ibm_db_check_sql_errors(conn_res->hdbc, SQL_HANDLE_DBC, 
												rc, 1, NULL, -1, 1);
				}
...
static void _python_ibm_db_add_param_cache( stmt_handle *stmt_res, int param_no, PyObject *var_pyvalue, int param_type, int size, SQLSMALLINT data_type, SQLINTEGER precision, SQLSMALLINT scale, SQLSMALLINT nullable )
{
...
static PyObject *_python_ibm_db_bind_param_helper(int argc, stmt_handle *stmt_res, SQLSMALLINT param_no, PyObject *var_pyvalue, long param_type, long data_type, long precision, long scale, long size)
{
...
//			rc = SQLDescribeParam((SQLHSTMT)stmt_res->hstmt, (SQLUSMALLINT)param_no, &sql_data_type, &sql_precision, &sql_scale, &sql_nullable);
			rc = SQLDescribeParam((SQLHSTMT)stmt_res->hstmt, (SQLSMALLINT)param_no, &sql_data_type, &sql_precision, &sql_scale, &sql_nullable);
			Py_END_ALLOW_THREADS;
...
//								(SQLUSMALLINT)param_no, &sql_data_type, 
								(SQLSMALLINT)param_no, &sql_data_type, 
...
//			sql_precision = (SQLUINTEGER)precision;
			sql_precision = (SQLINTEGER)precision;
			/* Add to cache */
			_python_ibm_db_add_param_cache(stmt_res, param_no, var_pyvalue,
											param_type, size, 
											sql_data_type, sql_precision, 
...
//			param_no = (SQLUSMALLINT) PyInt_AsLong(py_param_no);
			param_no = (SQLSMALLINT) PyInt_AsLong(py_param_no);
		} else {
			PyErr_SetString(PyExc_Exception, "Supplied parameter is invalid");
			return NULL;
		}
...
static PyObject *ibm_db_column_privileges(PyObject *self, PyObject *args)
{
	...
	if (!NIL_P(conn_res)) {
		...
		if (py_qualifier && py_qualifier != Py_None )
			qualifier = getUnicodeDataAsSQLCHAR(py_qualifier, &isNewBuffer);
		if (py_owner &&  py_owner != Py_None )
			owner = getUnicodeDataAsSQLCHAR(py_owner, &isNewBuffer);
		if (py_table_name && py_table_name != Py_None )
			table_name = getUnicodeDataAsSQLCHAR(py_table_name, &isNewBuffer);
...
static PyObject *ibm_db_columns(PyObject *self, PyObject *args)			
{
	...
	if (!NIL_P(conn_res)) {
		...
		if (py_column_name && py_column_name != Py_None )
			column_name = getUnicodeDataAsSQLCHAR(py_column_name, &isNewBuffer);
...
static PyObject *ibm_db_foreign_keys(PyObject *self, PyObject *args)
{
	...
	if (conn_res) {
		...
		if(py_pk_qualifier && py_pk_qualifier != Py_None)
			pk_qualifier = getUnicodeDataAsSQLCHAR(py_pk_qualifier, &isNewBuffer);
		if(py_pk_owner && py_pk_owner != Py_None)
			pk_owner = getUnicodeDataAsSQLCHAR(py_pk_owner, &isNewBuffer);
		if(py_pk_table_name && py_pk_table_name != Py_None)
			pk_table_name = getUnicodeDataAsSQLCHAR(py_pk_table_name, &isNewBuffer);
		if(py_fk_qualifier && py_fk_qualifier != Py_None)
			fk_qualifier = getUnicodeDataAsSQLCHAR(py_fk_qualifier, &isNewBuffer);
		if(py_fk_owner && py_fk_owner != Py_None)
			fk_owner = getUnicodeDataAsSQLCHAR(py_fk_owner, &isNewBuffer);
		if(py_fk_table_name && py_fk_table_name != Py_None)
			fk_table_name = getUnicodeDataAsSQLCHAR(py_fk_table_name, &isNewBuffer);
...
static PyObject *ibm_db_procedure_columns(PyObject *self, PyObject *args)
{
	...
	if (!NIL_P(conn_res)) {
		...
		if (py_proc_name && py_proc_name != Py_None )
			proc_name = getUnicodeDataAsSQLCHAR(py_proc_name, &isNewBuffer);
		if (py_column_name && py_column_name != Py_None )
			column_name = getUnicodeDataAsSQLCHAR(py_column_name, &isNewBuffer);
...
//		rc = SQLStatistics((SQLHSTMT)stmt_res->hstmt, qualifier, SQL_NTS, owner,
		rc = SQLStatistics((SQLHSTMT)stmt_res->hstmt, "", SQL_NTS, qualifier,
...
static PyObject *ibm_db_tables(PyObject *self, PyObject *args)
{
	...
	if (!NIL_P(conn_res)) {
		...
		if(py_table_type && py_table_type != Py_None)
			table_type = getUnicodeDataAsSQLCHAR(py_table_type, &isNewBuffer);
...
*/
static int _python_ibm_db_do_prepare(SQLHANDLE hdbc, SQLCHAR *stmt, int stmt_size, stmt_handle *stmt_res, PyObject *options)
{
	int rc;
...
//	printf(">>%d << %s  \n",stmt_size,stmt);
	Py_BEGIN_ALLOW_THREADS;
	rc = SQLPrepare((SQLHSTMT)stmt_res->hstmt, stmt, SQL_NTS); 
//				stmt_size);
...
static PyObject *ibm_db_exec(PyObject *self, PyObject *args)			
{
	...
	*/
	...
	if (!NIL_P(conn_res)) {
		...
		if (py_stmt != NULL && py_stmt != Py_None){
		    stmt = getUnicodeDataAsSQLCHAR(py_stmt, &isNewBuffer);
     		}
		
		Py_BEGIN_ALLOW_THREADS;
		rc = SQLExecDirect((SQLHSTMT)stmt_res->hstmt, stmt, SQL_NTS);
		Py_END_ALLOW_THREADS;
...
static param_node* build_list( stmt_handle *stmt_res, int param_no, SQLSMALLINT data_type, SQLINTEGER precision, SQLSMALLINT scale, SQLSMALLINT nullable )
{
		...
		case PYTHON_UNICODE:
			{
				...
				if(curr->data_type == SQL_BLOB || curr->data_type == SQL_BINARY || curr->data_type == SQL_VARBINARY) {
					...
					curr->uvalue = getUnicodeDataAsSQLCHAR(bind_data, &isNewBuffer);
					...
					curr->ivalue = curr->ivalue * sizeof(SQLCHAR);
				}
				if (curr->size != 0) {
					curr->ivalue = curr->size * sizeof(SQLCHAR);
				}
				if (isNewBuffer == 0 ){
					/* actually make a copy, since this will uvalue will be freed explicitly */
					SQLCHAR* tmp = (SQLCHAR*)ALLOC_N(SQLCHAR, curr->ivalue + 1);
					memcpy(tmp, curr->uvalue, (curr->ivalue + sizeof(SQLCHAR)));
					curr->uvalue = tmp;
				}
...
//	return PyInt_FromLong(stmt_res->cursor_type);
}
...
//static RETCODE _python_ibm_db_get_length(stmt_handle* stmt_res, SQLUSMALLINT col_num, SQLINTEGER *sLength)
static RETCODE _python_ibm_db_get_length(stmt_handle* stmt_res, SQLSMALLINT col_num, SQLINTEGER *sLength)
{
	RETCODE rc = SQL_SUCCESS;
	SQLHANDLE new_hstmt;
...
static RETCODE _python_ibm_db_get_data2(stmt_handle *stmt_res, SQLSMALLINT col_num, SQLSMALLINT ctype, SQLPOINTER buff, SQLLEN in_length, SQLINTEGER *out_length)
{
	RETCODE rc = SQL_SUCCESS;
	SQLHANDLE new_hstmt;
...
static PyObject *ibm_db_result(PyObject *self, PyObject *args)
{
	...
	long col_num;
	RETCODE rc;
	void	*out_ptr,*out_ptr2,*del_ptr;
	SQLCHAR *out_wchar_ptr;
	char error[DB2_MAX_ERR_MSG_LEN];
...
//						 out_ptr, in_length * sizeof(Py_UNICODE), &out_length);
			...
			if (out_length == SQL_NULL_DATA) {
				...
				return NULL;
				   ...
				   pch=(char *)out_ptr;
				   if (pch[0]=='.') {
				     out_ptr2 = (SQLPOINTER)ALLOC_N(Py_UNICODE, in_length + 1);
				     pch2=(char *)out_ptr2;
				     strcpy(pch2,"0");
				     strncat(pch2,pch,in_length);
				     del_ptr=out_ptr;
				     out_ptr=out_ptr2;
				     PyMem_Del(del_ptr);
				   }
				...
				if (column_type == SQL_TYPE_TIME) {
				  pch=(char *)out_ptr;
				  out_ptr2 = (SQLPOINTER)ALLOC_N(Py_UNICODE, in_length);
				  pch2=(char *)out_ptr2;
				  strcpy(pch2,pch);
				...
				if (column_type == SQL_TYPE_TIMESTAMP) {
				  pch=(char *)out_ptr;
				  out_ptr2 = (SQLPOINTER)ALLOC_N(Py_UNICODE, in_length);
				  pch2=(char *)out_ptr2;
				  strcpy(pch2,pch);
...
//				if (column_type == SQL_TYPE_TIMESTAMP)
//				    return_value = getSQLCHARAsPyUnicodeObject(out_ptr, out_length);
//				else
...
//			rc = _python_ibm_db_get_length(stmt_res, (SQLUSMALLINT) col_num+1, &in_length);
			rc = _python_ibm_db_get_length(stmt_res, (SQLSMALLINT) col_num+1, &in_length);
			if ( rc == SQL_ERROR ) {
				Py_INCREF(Py_False);
				return Py_False;
			}
			...
			out_wchar_ptr = (SQLCHAR*)ALLOC_N(SQLCHAR, in_length+1);
...
//						  out_wchar_ptr, (in_length+1) * sizeof(SQLCHAR), 
//						  &out_length);
    			rc = _python_ibm_db_get_data2(stmt_res, (SQLSMALLINT) col_num+1, SQL_C_CHAR, 
						  out_wchar_ptr, (in_length+1) * sizeof(SQLCHAR), 
						  &out_length);
			...
			retVal = getSQLCHARAsPyUnicodeObject(out_wchar_ptr, in_length * sizeof(SQLCHAR));
//			retVal = PyString_FromString((char *)out_wchar_ptr);
...
//					rc = _python_ibm_db_get_data2(stmt_res, (SQLUSMALLINT)col_num + 1, 
					rc = _python_ibm_db_get_data2(stmt_res, (SQLSMALLINT)col_num + 1, 
...
//					0, (SQLINTEGER *)&in_length);
			rc = _python_ibm_db_get_data(stmt_res, (SQLSMALLINT)col_num + 1, SQL_C_CHAR, NULL, 
					0, (SQLINTEGER *)&in_length);
			if ( rc == SQL_ERROR ) {
				Py_INCREF(Py_False);
				return Py_False;
			...
			out_ptr = (SQLPOINTER)ALLOC_N(SQLCHAR, in_length);
			if ( out_ptr == NULL ) {
				PyErr_SetString(PyExc_Exception, 
...
//			rc = _python_ibm_db_get_data2(stmt_res, (SQLUSMALLINT)col_num + 1, SQL_C_BINARY, 
			rc = _python_ibm_db_get_data2(stmt_res, (SQLSMALLINT)col_num + 1, SQL_C_BINARY, 
						  (SQLPOINTER)out_ptr, in_length * sizeof(SQLCHAR), 
						  &out_length);
			if (rc == SQL_ERROR) {
				if(out_ptr != NULL) {
					PyMem_Del(out_ptr);
...
//  			retVal = getSQLCHARAsPyUnicodeObject((SQLCHAR*)out_ptr, out_length);
			retVal = PyString_FromString((char *)out_ptr);
...
//MAX evade django TIMESTAMP		
//		if (column_type == SQL_TYPE_TIMESTAMP) {stmt_res->column_info[column_number].type=SQL_CHAR;}
//  	        if (column_type == SQL_TIME) {stmt_res->column_info[column_number].type=SQL_CHAR;}
...
//					(SQLCHAR*)strtolower2((char*)stmt_res->column_info[column_number].name, 
//					strlen((char*)stmt_res->column_info[column_number].name));
			...
			case CASE_UPPER:
//				stmt_res->column_info[column_number].name = 
//					(SQLCHAR*)strtoupper2((char*)stmt_res->column_info[column_number].name, 
//					strlen((char*)stmt_res->column_info[column_number].name));
				stmt_res->column_info[column_number].name = 
...
//				  	value = getSQLCHARAsPyUnicodeObject(row_data->str_val, out_length);
...
//                        	case SQL_TYPE_TIMESTAMP:
				...
				case SQL_LONGVARCHAR:
					...
					wout_ptr = (SQLPOINTER)malloc(tmp_length * sizeof(SQLCHAR) + 1);
					if ( wout_ptr == NULL ) {
						PyErr_SetString(PyExc_Exception, "Failed to Allocate Memory");
						return NULL;
					}
...
//						(tmp_length * sizeof(SQLCHAR) + 1), &out_length);
  					rc = _python_ibm_db_get_data(stmt_res, column_number + 1, SQL_C_CHAR, wout_ptr,
						(tmp_length * sizeof(SQLCHAR) + 1), &out_length);
...
//  						value = getSQLCHARAsPyUnicodeObject(wout_ptr, out_length);
						value = PyString_FromString((char *)wout_ptr);
				...
				case SQL_DECFLOAT:
	    				if (column_type == SQL_DECIMAL || column_type == SQL_NUMERIC){
					   pch=(char *)row_data->str_val;
					   if (pch[0]=='.') {
					     tmp_length = stmt_res->column_info[column_number].size;
					     wout_ptr = (SQLPOINTER)malloc(tmp_length * sizeof(SQLCHAR));
					   
					     pch2=(char *)wout_ptr;
					     strcpy(pch2,"0");
					     strncat(pch2,pch,tmp_length);
					     value= PyString_FromString((char*)wout_ptr);
					     free(wout_ptr);
					     pch2=NULL;
					     pch=NULL;
...
//					0, &tmp_length);
				rc = _python_ibm_db_get_data(stmt_res, column_number + 1, SQL_C_CHAR, NULL, 
					0, &tmp_length);
				...
				if (tmp_length == SQL_NULL_DATA) {
					...
					wout_ptr = (SQLCHAR *)ALLOC_N(SQLCHAR, tmp_length);
...
//						wout_ptr, tmp_length * sizeof(SQLCHAR) , &out_length);
					rc = _python_ibm_db_get_data(stmt_res, column_number + 1, SQL_C_CHAR, 
						wout_ptr, tmp_length * sizeof(SQLCHAR) , &out_length);
...
//  					value = getSQLCHARAsPyUnicodeObject(wout_ptr, out_length);
			...
			case SQL_CLOB:
				...
				if (tmp_length == SQL_NULL_DATA) {
					...
					if (rc == SQL_ERROR) tmp_length = 0;
					wout_ptr = (SQLPOINTER)ALLOC_N(SQLCHAR, tmp_length + 1);
					if ( wout_ptr == NULL ) {
						PyErr_SetString(PyExc_Exception, 
							"Failed to Allocate Memory for LOB Data");
						return NULL;
...
//						wout_ptr , (tmp_length + 1) * sizeof(SQLCHAR), 
//						&out_length);
					...
					value = getSQLCHARAsPyUnicodeObject(wout_ptr, (tmp_length) * sizeof(SQLCHAR));
//					value = PyString_FromString((char *)wout_ptr);
...
//									PyTuple_SetItem(outTuple, paramCount, getSQLCHARAsPyUnicodeObject(tmp_curr->uvalue, tmp_curr->bind_indicator));
									
									PyTuple_SetItem(outTuple, paramCount, PyString_FromString(tmp_curr->uvalue));
...
//		rc = SQLGetFunctions(conn_res->hdbc, (SQLUSMALLINT) funtion_id, (SQLUSMALLINT*) &supported);
		rc = SQLGetFunctions(conn_res->hdbc, (SQLSMALLINT) funtion_id, (SQLSMALLINT*) &supported);
